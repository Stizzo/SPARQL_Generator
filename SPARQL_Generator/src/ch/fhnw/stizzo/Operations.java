...
public class Operations {
	...
	private ArrayList<String> ontologyPreamble;					//it contains all the lines that occurs between the start of the file (ttl) and the start of the first ontology object
	private ArrayList<ADOxxDecisionTable> adoxxDecisionTables; 	//it contains all the ADOxx decision tables
	...
	public int[] parseOntology(String path_file,  boolean offline){//this method parse the ontology
	...
	public int[] loadXML(String path_file){//this method load the xml file with the ADOxx DecisionTable(s) and instances
				...
				if (models.getChildNodes().item(j).getNodeName().equals("MODEL")){
					...
					for (int k = 0; k < model.getChildNodes().getLength(); k++){
						...
						if (model.getChildNodes().item(k).getNodeName().equals("INSTANCE")){
							...
							if (!instance.getAttributes().getNamedItem("class").getNodeValue().equals("Boxed Expression")){
								/*
								//here it is checking if the class of the adoxx instance is present in the excel file
								for (int y = 0; y < objectMatches.size();y++){
									if (instance.getAttributes().getNamedItem("class").getNodeValue().equals(objectMatches.get(y).getAdoxxAttributeName())
											&& objectMatches.get(y).getAdoxxType().trim().equals("Class")){
							
							//here we are parsing a non-decision table instance	
							String inst_id = instance.getAttributes().getNamedItem("id").getNodeValue();
							String inst_class = instance.getAttributes().getNamedItem("class").getNodeValue();
							String inst_name = instance.getAttributes().getNamedItem("name").getNodeValue();
							ArrayList<ADOxxAttribute> inst_attributes = new ArrayList<ADOxxAttribute>();; //this arraylist is used to create the object
							ADOxxAttribute temp_attributes_to_save = null;
						
						
							ArrayList<Node> temp_attributes = new ArrayList<Node>(); //arraylist where to collect all the <ATTRIBUTES> of the XML of an object
							for (int z = 0; z < instance.getChildNodes().getLength(); z++){
								if (instance.getChildNodes().item(z).getNodeName().equals("ATTRIBUTE")){
									//i save all the node ATTRIBUTE in an array called temp_attributes
									temp_attributes.add(instance.getChildNodes().item(z));
				
								}
							}
							
							for (int l = 0; l < temp_attributes.size();l++){
							
								for (int m = 0; m < objectMatches.size(); m++){
									
									if (temp_attributes.get(l).getAttributes().getNamedItem("name").getNodeValue().equals(objectMatches.get(m).getAdoxxAttributeName())
											&& inst_class.equals(objectMatches.get(m).getAdoxxType())){

									if (temp_attributes.get(l).getChildNodes().getLength() == 0){ //Checking for a non-null value;
										temp_attributes_to_save = new ADOxxAttribute(
												temp_attributes.get(l).getAttributes().getNamedItem("name").getNodeValue(),
												temp_attributes.get(l).getAttributes().getNamedItem("type").getNodeValue(),
												"");
										inst_attributes.add(temp_attributes_to_save);
						
									} else {
										temp_attributes_to_save = new ADOxxAttribute(
												temp_attributes.get(l).getAttributes().getNamedItem("name").getNodeValue(),
												temp_attributes.get(l).getAttributes().getNamedItem("type").getNodeValue(),
												temp_attributes.get(l).getLastChild().getNodeValue());
										inst_attributes.add(temp_attributes_to_save);
									}
							...
							adoxxInstances.add(new ADOxxInstance(inst_id,inst_class,inst_name,inst_attributes));
							}
						...
						*/
								...
								String DT_name;
								String hit_policy = null;	//variable for ADOxxDecisionTable
								String aggregation_indicator = null;	//variable for ADOxxDecisionTable
								...
								for (int z = 0; z < instance.getChildNodes().getLength(); z++){
									if (instance.getChildNodes().item(z).getNodeName().equals("ATTRIBUTE")){
										...
										temp_attributes.add(instance.getChildNodes().item(z));
												...
												instance.getChildNodes().item(z).getAttributes().item(0).getNodeValue().equals("Decision Table GraphRep")) { //we get the item(0) because we suppose that the item(0) is the "name=" attribute
										temp_record = instance.getChildNodes().item(z); //here we found the decision table node <RECORD>
								...
								for (int a = 0; a <temp_attributes.size(); a++){
									...
									for (int b = 0; b < temp_attributes.get(a).getAttributes().getLength(); b++){
										...
										if (temp_attributes.get(a).getAttributes().item(b).getNodeValue().equals("Hit Policy")){
											temp_node = temp_attributes.get(a);
											hit_policy = temp_node.getLastChild().getNodeValue();
									
										}
										if (temp_attributes.get(a).getAttributes().item(b).getNodeValue().equals("Aggregation Indicator")){
											temp_node = temp_attributes.get(a);
											aggregation_indicator = temp_node.getLastChild().getNodeValue();
									
										}
										if (temp_attributes.get(a).getAttributes().item(b).getNodeValue().equals("Priority of Execution")){
											temp_node = temp_attributes.get(a);
											if (temp_node.getLastChild().getNodeValue().equals("")){
												priority_of_execution = 0;
											}else{
												priority_of_execution = Integer.parseInt(temp_node.getLastChild().getNodeValue());
								...
								ArrayList<ADOxxDecisionTableEntry> input_names = new ArrayList<ADOxxDecisionTableEntry>();	//arraylist for ADOxxDecisionTable
								ArrayList<ADOxxDecisionTableEntry> output_names = new ArrayList<ADOxxDecisionTableEntry>();	//arraylist for ADOxxDecisionTable
								ArrayList<ADOxxDecisionTableRow> rows = new ArrayList<ADOxxDecisionTableRow>();	//arraylist for ADOxxDecisionTable
								int num_row = 0;		//1 means names of variable; 2 and more means  data
								int num_rule = 0;			//variable for DTRow
								ArrayList<String> input = null;	//arraylist for DTRow
								ArrayList<String> output = null;	//arraylist for DTRow
								Node temp_row = null;
								Node temp_attribute = null;
										
								for (int a = 0; a < temp_record.getChildNodes().getLength(); a++){ //iterating all the rows
									if (temp_record.getChildNodes().item(a).getNodeName().equals("ROW")){
									temp_row = temp_record.getChildNodes().item(a);
									input = new ArrayList<String>();
									output = new ArrayList<String>();
									
									for (int b = 0; b < temp_row.getAttributes().getLength();b++){ //iterating attributes of the row
										if (temp_row.getAttributes().item(b).getNodeName().equals("number")){
											num_row = Integer.parseInt(temp_row.getAttributes().item(b).getNodeValue());
										
										}	
									} //end iterating attributes of the row
									
									for (int c = 0; c < temp_row.getChildNodes().getLength(); c++){	//iterating all the ATTRIBUTE of ROW
										if (temp_row.getChildNodes().item(c).getNodeName().equals("ATTRIBUTE")){
											temp_attribute = temp_row.getChildNodes().item(c);
									
											if (num_row == 1){		//parsing name of values
											
												if (temp_attribute.getChildNodes().getLength() !=0){	//chicking for no-null values
													if (temp_attribute.getAttributes().item(0).getNodeValue().startsWith("Input")){
														
														String[] inputSplittate = temp_attribute.getLastChild().getNodeValue().trim().replaceAll(" ", "_").split("\\.");
														
														ArrayList<String> temp_properties = new ArrayList<String>();
														
														String temp_obj_name = inputSplittate[0];
														String temp_dest_name = inputSplittate[inputSplittate.length-1];;
														for (int x = 1 ; x < inputSplittate.length-1;x++){
															temp_properties.add(inputSplittate[x]);
														}
														input_names.add(new ADOxxDecisionTableEntry(temp_obj_name, temp_properties, temp_dest_name));
													}else if(temp_attribute.getAttributes().item(0).getNodeValue().startsWith("Output")){
														String[] mainSplit = temp_attribute.getLastChild().getNodeValue().trim().replaceAll(" ","_").split("\\|");
														if (mainSplit.length == 2){
															int output_priority = Integer.parseInt(mainSplit[0]);
															String[] outputSplittate = mainSplit[1].trim().replaceAll(" ","_").split("\\.");
															ArrayList<String> temp_properties = new ArrayList<String>();
															temp_properties.add(outputSplittate[1]);
															String temp_obj_name = outputSplittate[0];
															String temp_dest_name = outputSplittate[2];;
															
															output_names.add(new ADOxxDecisionTableEntry(temp_obj_name, temp_properties, temp_dest_name, output_priority));
														}else{
															String[] outputSplittate = mainSplit[0].trim().replaceAll(" ","_").split("\\.");
															ArrayList<String> temp_properties = new ArrayList<String>();
															temp_properties.add(outputSplittate[1]);
															String temp_obj_name = outputSplittate[0];
															String temp_dest_name = outputSplittate[2];;
															
															output_names.add(new ADOxxDecisionTableEntry(temp_obj_name, temp_properties, temp_dest_name));
														}
														
													}
													num_rule = -1;
												}//end if checking for no-null values
												
												}else if(num_row >= 1){	//parsing data
												if (temp_attribute.getChildNodes().getLength() !=0){	//chicking for no-null values
													
													if (temp_attribute.getAttributes().item(0).getNodeValue().startsWith("Input")){
														input.add(temp_attribute.getLastChild().getNodeValue().replaceAll("&quot;","\""));
														//System.out.println("input: "+temp_attribute.getLastChild().getNodeValue());
													} else if (temp_attribute.getAttributes().item(0).getNodeValue().startsWith("Output")){
														output.add(temp_attribute.getLastChild().getNodeValue().replaceAll("&quot;","\""));
														//System.out.println("output: "+temp_attribute.getLastChild().getNodeValue());
													} else if (temp_attribute.getAttributes().item(0).getNodeValue().startsWith("Rule")){
														num_rule = Integer.parseInt(temp_attribute.getLastChild().getNodeValue());
														//System.out.println("rule number: "+temp_attribute.getLastChild().getNodeValue());
													}
												
												}//end if checking for no-null values
											}//end checking number of row	
											
										}	//end if we are analyzing an ATTRIBUTE
								...
								adoxxDecisionTables.add(new ADOxxDecisionTable(DT_name,hit_policy, aggregation_indicator, input_names,output_names, rows, priority_of_execution));
							}
	...
	/*for (int i = 0; i < adoxxInstances.size(); i++){
		System.out.println("------------------------");
		System.out.println("ID: "+adoxxInstances.get(i).getInst_id()+"    Name: "+adoxxInstances.get(i).getInst_name()+"    Type: "+adoxxInstances.get(i).getInst_class()+"    #Instances: "+adoxxInstances.get(i).getAttributes().size());
		for (int j = 0; j < adoxxInstances.get(i).getAttributes().size(); j++){
			System.out.println("    Name: "+adoxxInstances.get(i).getAttributes().get(j).get(0) + " - Type: "+adoxxInstances.get(i).getAttributes().get(j).get(1) + " - Value: "+adoxxInstances.get(i).getAttributes().get(j).get(2));
		}
	}
	*/
	//TEST, DECISION TABLE ONLY (only the first decision table)
	/*System.out.println("Decision table: " + adoxxDecisionTables.get(0).getHit_policy());
	System.out.println("Number of names: "+ adoxxDecisionTables.get(0).getInput_names().size());
	for (int i = 0; i < adoxxDecisionTables.get(0).getInput_names().size(); i++){
		System.out.println("    "+adoxxDecisionTables.get(0).getInput_names().get(i));
	}
	System.out.println("Number of types: "+ adoxxDecisionTables.get(0).getInput_types().size());
	for (int i = 0; i < adoxxDecisionTables.get(0).getInput_types().size(); i++){
		System.out.println("    "+adoxxDecisionTables.get(0).getInput_types().get(i));
	}
	System.out.println("Number of rules: "+ adoxxDecisionTables.get(0).getRows().size());
	
	for (int i = 0; i < adoxxDecisionTables.get(0).getRows().size(); i++){
		System.out.println("Rule number: "+ adoxxDecisionTables.get(0).getRows().get(i).num_row);
		System.out.println("    inputs:");
		for (int j = 0; j < adoxxDecisionTables.get(0).getRows().get(i).getInput().size(); j++){
				System.out.println("        "+adoxxDecisionTables.get(0).getRows().get(i).getInput().get(j));
		
		
	}
	for (int k = 0; k < adoxxDecisionTables.get(0).getRows().get(i).getOutput().size(); k++){
		
		System.out.println("    outputs:");
		
		System.out.println("        "+adoxxDecisionTables.get(0).getRows().get(i).getOutput().get(k));
		
	}
	}
	*/
	return new int[]{numb_model,adoxxDecisionTables.size()}; //models,instances,decision tables
	}

	public void patchXML(String path_file, boolean offline){//this method remove the DOCTYPE tag from the xml file and re-save the file without it
	...
	public void printDecisionTables(){
		for (int i = 0; i < adoxxDecisionTables.size();i++){
			System.out.print(adoxxDecisionTables.get(i).getName()+ " - " + "Hit Policy: "+adoxxDecisionTables.get(i).getHit_policy() + " - " + adoxxDecisionTables.get(i).getAggregation_indicator()+ " with " + adoxxDecisionTables.get(i).getOutput_names().size() + " outputs and priority " + adoxxDecisionTables.get(i).getPriority());
	...
	public void executeDecisionTables(){
		...
		try {
			if (test.windowPrefs.getChkRenameTheOutput().isSelected()){
				writer = new PrintWriter(test.ui.getTxtOutputDirectory().getText()+File.separator+output_file+System.currentTimeMillis()+".txt", "UTF-8");
				writer_status = new PrintWriter(test.ui.getTxtOutputDirectory().getText()+File.separator+status_file+System.currentTimeMillis()+".txt", "UTF-8");
			}else{
				writer = new PrintWriter(test.ui.getTxtOutputDirectory().getText()+File.separator+output_file+".txt", "UTF-8");
				writer_status = new PrintWriter(test.ui.getTxtOutputDirectory().getText()+File.separator+status_file+".txt", "UTF-8");
			}
	...
	adoxxDecisionTables = sorted_DT;
		...
		for (int i = 0; i < adoxxDecisionTables.size(); i++){
			...
			ArrayList<ADOxxDecisionTableNormalizedEntry> temp_input_entries = normalizeInputEntries(DT.getInput_names());
			ArrayList<ADOxxDecisionTableNormalizedEntry> temp_output_entries = normalizeInputEntries(DT.getOutput_names());
			// TEST FOR NORMALIZED INPUT ENTRY
			/* 
			 for (int k = 0; k < temp_input_entries.size();k++){
				System.out.println(temp_input_entries.get(k).getObject_name() + " - " + temp_input_entries.get(k).getProperty() + " => " + temp_input_entries.get(k).getDest_name() + " ("+temp_input_entries.get(k).getType()+")");
			}
			System.out.println("-----------------");
			*/
			...
			for (int j = 0; j < temp_input_entries.size(); j++){
				if (!temp_input_entries.get(j).getObject_name().startsWith("?") &&
						!variableIsInList(temp_input_entries.get(j).getObject_name())){
								variables.add(new VariableMatch(
	...
	//***********************
				...
				writer.println("CONSTRUCT {");
					...
					if (DT.getOutput_names().get(temp_output_entries.get(j).getNum_entry()).getObject_name().startsWith("?")){
						writer.print(DT.getOutput_names().get(temp_output_entries.get(j).getNum_entry()).getObject_name());
					} else {
						for (int x = 0; x < variables.size(); x++){
							if (variables.get(x).getOntologyItem().getNameWithoutPrefix().equals(DT.getOutput_names().get(temp_output_entries.get(j).getNum_entry()).getObject_name())){
								writer.print(variables.get(x).getVariableName());
							}
						}
					}
					//HERE THERE IS THE LIMITATION OF THE ONE PROPERTY ON THE OUTPUT ENTRY
				writer.print(" " + getPropertyFromString(DT.getOutput_names().get(temp_output_entries.get(j).getNum_entry()).getProperties().get(0)).getName() + " " +
				...
				for (int b = 0; b < variables.size(); b++){
					writer.println("?"+variables.get(b).getVariableName() + " rdf:type " + variables.get(b).getOntologyItem().getName() + " . ");
				}
				...
				for (int y = 0; y < temp_input_entries.size(); y++){
					if (temp_input_entries.get(y).getIsARelation()){
						writer.print("OPTIONAL {" + 
								temp_input_entries.get(y).getObject_name() + " " +
				...
				for (int c = 0; c < temp_input_entries.size(); c++){
					if (temp_input_entries.get(c).getHaveValue() &&
						...
						writer.print("OPTIONAL {" + 
								temp_input_entries.get(c).getObject_name() + " ");
				...
				//DECLARATIONS OF OUTPUT
					...
					//IF THE OUTPUT IS NOT A DATATYPE, WE NEED TO DEFINE THE LABEL OF THE DESTINATION OBJECT
					...
					if (temp_output_entries.get(j).getHaveValue()){
						...
						for (int k = 0; k < DT.getRows().size(); k++){
							...
							if (!DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry()).equals("-")){
							...
							writer.print("IF (");
								...
								for (int l = 0; l < temp_input_entries.size(); l++){
									...
									if (temp_input_entries.get(l).getHaveValue() &&
											!DT.getRows().get(k).getInput().get(temp_input_entries.get(l).getNum_entry()).equals("-")){
										if (!firstInput){
											writer.print(" && ");
										}
										
										//WRITING THE DESTINATION INPUT CRITERIA
										writer.print(temp_input_entries.get(l).getDest_name());
										//WRITING THE OPERATOR OF INPUT CRITERIA
										String[] arraySplittate = DT.getRows().get(k).getInput().get(temp_input_entries.get(l).getNum_entry()).split(" ");
										if (arraySplittate.length == 2 && 
												arraySplittate[0].startsWith("=") ||
												arraySplittate[0].startsWith(">") ||
												arraySplittate[0].startsWith("<")){
											//Here is parsing a number/date operator
											writer.print(arraySplittate[0]);
										} else {
											//Here is parsing a non number/date operator
											writer.print(" = ");
										}
										...
										if (arraySplittate.length == 2 &&
												...
												arraySplittate[0].startsWith(">") ||
												arraySplittate[0].startsWith("<")){
											//Here is writing a date/number value
											writer.print(arraySplittate[1]);
												} else if (temp_input_entries.get(l).getIsARelation()) {
													writer.print(getInstanceFromString(DT.getRows().get(k).getInput().get(temp_input_entries.get(l).getNum_entry())).getName());
												}else {
											//Here is writing a "string" or value
													writer.print(DT.getRows().get(k).getInput().get(temp_input_entries.get(l).getNum_entry()));
												}
								...
								if (!DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry()).equals("-")){
									...
									//HERE THERE IS THE LIMIT OF 1 PROPERTY IN THE OUTPUT NAME
								if (getPropertyFromString(DT.getOutput_names().get(temp_output_entries.get(j).getNum_entry()).getProperties().get(0)).isAnObjectProperty()){
									//Here it is expecting to find an instance as output
									writer.print(getInstanceFromString(DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry())).getName());	
								}else{ 
									//Here it is expeecting to find a datatype value as output
	...
	//Need to check the consistency of the Decision Table
					...
					writer_status.println("WARNING: Cannot check the consistency for the Decision Table \""+adoxxDecisionTables.get(i).getName() + "\" with the Hit Policy \"Single Hit Any\" (A)");
	...
	//***********************
				ArrayList<ADOxxDecisionTableNormalizedEntry> sorted_output = new ArrayList<ADOxxDecisionTableNormalizedEntry>();
				while (temp_output_entries.size()!= 0){
					int id = -100;
					int min = 1000;
					for (int h = 0; h < temp_output_entries.size(); h++){
						if (temp_output_entries.get(h).getOutput_priority()<min){
							id = h;
							min = temp_output_entries.get(h).getOutput_priority();
						}
						}
					sorted_output.add(temp_output_entries.get(id));
					temp_output_entries.remove(id);
				}
				
			temp_output_entries = sorted_output;	
	...
	//***********************
		...
		//===============
					...
					if (temp_output_entries.get(j).getHaveValue()){
						...
						for (int k = 0; k < DT.getRows().size(); k++){
							//IF THE OUTPUT IS DIFFERENT FROM "-"
							if (!DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry()).equals("-") &&
									!(""+k).equals(alternative_rule_index)){
								boolean firstInput = true;	
								writer.print("BIND(");	
								writer.print("IF (");
								num_input++;
							...
							if (!firstRow){
							writer.print(" && ?MIN"+(num_input-1)+" > " + DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry()));
							}
							...
							writer.print(", ");
							if (firstRow){
								//HERE I WRITE THE ALTERNATIVE VALUE IF IT EXISTS
		...
		//===============
				...
				if (temp_output_entries.get(j).getHaveValue()){
					...
					for (int k = 0; k < DT.getRows().size(); k++){
						...
						if (!firstRow){
						writer.print(" && ?MAX"+(num_input-1)+" < " + DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry()));
						}
		...
		//===============
				...
				if (temp_output_entries.get(j).getHaveValue()){
					...
					for (int k = 0; k < DT.getRows().size(); k++){
						...
						if (getPropertyFromString(DT.getOutput_names().get(temp_output_entries.get(j).getNum_entry()).getProperties().get(0)).isAnObjectProperty()){
							//Here it is expecting to find an instance as output
							writer.print("?SUM"+(num_input-1)+"+"+getInstanceFromString(DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry())).getName());	
						}else{
							if (firstRow){
								writer.print(DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry()));
							}else{
								writer.print("?SUM"+(num_input-1)+"+"+DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry()));
							}
							//Here it is expeecting to find a datatype value as output
							//writer.print(DT.getRows().get(k).getOutput().get(temp_output_entries.get(j).getNum_entry()));
		...
		//===============
				...
				writer_status.println("ERROR: AGGREGATOR not found/detected for the Decision Table \""+DT.getName()+"\"");
			}
				...
				writer_status.println("ERROR: HIT POLICY not found/detected for the Decision Table \""+DT.getName()+"\"");
			}
				
		}
		writer.close();
		writer_status.close();
		JOptionPane.showMessageDialog(null, "Decision Tables transformed with success!","Success!",JOptionPane.INFORMATION_MESSAGE);
	}
	...
	private ArrayList<ADOxxDecisionTableNormalizedEntry> normalizeInputEntries(ArrayList<ADOxxDecisionTableEntry> entries){
		...
		for (int i = 0; i < entries.size(); i++){
			if (entries.get(i).getProperties().size() == 1){
				...
				if (getPropertyFromString(entries.get(i).getProperties().get(0)).isAnObjectProperty()){
					...
					result.add(new ADOxxDecisionTableNormalizedEntry(
							entries.get(i).getObject_name(),
							entries.get(i).getProperties().get(0),
							entries.get(i).getDest_name(),
							true,
							true,
							i,
							entries.get(i).getOutput_priority()));
				...
				for (int j = 0; j < entries.get(i).getProperties().size(); j++){
					...
					if ( j != entries.get(i).getProperties().size()-1){
						result.add(new ADOxxDecisionTableNormalizedEntry(
								next_source,
								entries.get(i).getProperties().get(j),
								"?"+i+"temp"+j,
								true,
								false));
	...
	public void testing(){
		...
		for (int i = 0; i < adoxxDecisionTables.size(); i++)
		{
			...
			for (int j = 0 ; j < temp_input_entries.size(); j++){
				System.out.println("Relation: "+temp_input_entries.get(j).getIsARelation() + " / " +
						"Input: " + temp_input_entries.get(j).getHaveValue() + " - " +
						temp_input_entries.get(j).getNum_entry() +" - " + 
						temp_input_entries.get(j).getObject_name() + " - " +
						temp_input_entries.get(j).getProperty() + " - " +
						temp_input_entries.get(j).getDest_name()
						);
			}
			System.out.println("-------------");
			for (int j = 0 ; j < temp_output_entries.size(); j++){
				System.out.println("Relation: "+temp_output_entries.get(j).getIsARelation() + " / " +
						"Input: " + temp_output_entries.get(j).getHaveValue() + " - " +
						temp_output_entries.get(j).getNum_entry() +" - " + 
						temp_output_entries.get(j).getObject_name() + " - " +
						temp_output_entries.get(j).getProperty() + " - " +
						temp_output_entries.get(j).getDest_name()
						);
			}
			System.out.println("===========");
	...
	/*public void performConstructRule(ParameterizedSparqlString queryStr) {
		  ...
		  QueryExecution qexec = QueryExecutionFactory.create(queryStr.toString(), rdfModel);
